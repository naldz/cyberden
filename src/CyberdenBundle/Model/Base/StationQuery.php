...
use CyberdenBundle\Model\Map\StationTableMap;
 ...
 * @method     ChildStationQuery orderByMacAddress($order = Criteria::ASC) Order by the mac_address column
 * @method     ChildStationQuery orderByIsCommissioned($order = Criteria::ASC) Order by the is_commissioned column
 *
 * @method     ChildStationQuery groupById() Group by the id column
 * @method     ChildStationQuery groupByName() Group by the name column
 * @method     ChildStationQuery groupByMacAddress() Group by the mac_address column
 * @method     ChildStationQuery groupByIsCommissioned() Group by the is_commissioned column
 *
 ...
 *
 * @method     ChildStationQuery leftJoinSession($relationAlias = null) Adds a LEFT JOIN clause to the query using the Session relation
 * @method     ChildStationQuery rightJoinSession($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Session relation
 * @method     ChildStationQuery innerJoinSession($relationAlias = null) Adds a INNER JOIN clause to the query using the Session relation
 *
 * @method     ChildStationQuery joinWithSession($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Session relation
 *
 * @method     ChildStationQuery leftJoinWithSession() Adds a LEFT JOIN clause and with to the query using the Session relation
 * @method     ChildStationQuery rightJoinWithSession() Adds a RIGHT JOIN clause and with to the query using the Session relation
 * @method     ChildStationQuery innerJoinWithSession() Adds a INNER JOIN clause and with to the query using the Session relation
 *
 ...
 * @method     ChildStation findOneByMacAddress(string $mac_address) Return the first ChildStation filtered by the mac_address column
 * @method     ChildStation findOneByIsCommissioned(int $is_commissioned) Return the first ChildStation filtered by the is_commissioned column *
 ...
 *
 * @method     ChildStation requireOneById(int $id) Return the first ChildStation filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildStation requireOneByMacAddress(string $mac_address) Return the first ChildStation filtered by the mac_address column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildStation requireOneByIsCommissioned(int $is_commissioned) Return the first ChildStation filtered by the is_commissioned column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildStation[]|ObjectCollection findByMacAddress(string $mac_address) Return ChildStation objects filtered by the mac_address column
 * @method     ChildStation[]|ObjectCollection findByIsCommissioned(int $is_commissioned) Return ChildStation objects filtered by the is_commissioned column
...
abstract class StationQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \CyberdenBundle\Model\Base\StationQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'cyberden', $modelName = '\\CyberdenBundle\\Model\\Station', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, mac_address, is_commissioned FROM station WHERE id = :p0';
        try {
    ...
    public function filterByMacAddress($macAddress = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(StationTableMap::COL_MAC_ADDRESS, $macAddress, $comparison);
    }

    /**
     * Filter the query on the is_commissioned column
     *
     * Example usage:
     * <code>
     * $query->filterByIsCommissioned(1234); // WHERE is_commissioned = 1234
     * $query->filterByIsCommissioned(array(12, 34)); // WHERE is_commissioned IN (12, 34)
     * $query->filterByIsCommissioned(array('min' => 12)); // WHERE is_commissioned > 12
     * </code>
     *
     * @param     mixed $isCommissioned The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildStationQuery The current query, for fluid interface
     */
    public function filterByIsCommissioned($isCommissioned = null, $comparison = null)
    {
        if (is_array($isCommissioned)) {
            $useMinMax = false;
            if (isset($isCommissioned['min'])) {
                $this->addUsingAlias(StationTableMap::COL_IS_COMMISSIONED, $isCommissioned['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isCommissioned['max'])) {
                $this->addUsingAlias(StationTableMap::COL_IS_COMMISSIONED, $isCommissioned['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(StationTableMap::COL_IS_COMMISSIONED, $isCommissioned, $comparison);
    }
    ...
    public function filterBySession($session, $comparison = null)
    {
        if ($session instanceof \CyberdenBundle\Model\Session) {
            return $this
                ->addUsingAlias(StationTableMap::COL_ID, $session->getStationId(), $comparison);
