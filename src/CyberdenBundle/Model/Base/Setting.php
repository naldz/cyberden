...
abstract class Setting implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\CyberdenBundle\\Model\\Map\\SettingTableMap';
    ...
    /**
     * The value for the rental_price field.
     *
     * @var        int
     */
    ...
    /**
     * Initializes internal state of CyberdenBundle\Model\Base\Setting object.
     */
    ...
    /**
     * Get the [rental_price] column value.
     *
     * @return int
     */
     ...
     * @return $this|\CyberdenBundle\Model\Setting The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [rental_price] column.
     *
     * @param int $v new value
     * @return $this|\CyberdenBundle\Model\Setting The current object (for fluent API support)
     */
    public function setRentalPrice($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->rental_price !== $v) {
            $this->rental_price = $v;
            $this->modifiedColumns[SettingTableMap::COL_RENTAL_PRICE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SettingTableMap::translateFieldName('RentalPrice', TableMap::TYPE_PHPNAME, $indexType)];
            $this->rental_price = (null !== $col) ? (int) $col : null;
            $this->resetModified();
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\CyberdenBundle\\Model\\Setting'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SettingTableMap::COL_RENTAL_PRICE)) {
            $modifiedColumns[':p' . $index++]  = 'rental_price';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':
                        ...
                        break;
                    case 'rental_price':
                        $stmt->bindValue($identifier, $this->rental_price, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\CyberdenBundle\Model\Setting
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setRentalPrice($arr[$keys[1]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\CyberdenBundle\Model\Setting The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SettingTableMap::COL_RENTAL_PRICE)) {
            $criteria->add(SettingTableMap::COL_RENTAL_PRICE, $this->rental_price);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \CyberdenBundle\Model\Setting (or compatible) type.
    ...
    {
        $copyObj->setRentalPrice($this->getRentalPrice());
        if ($makeNew) {
            $copyObj->setNew(true);
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \CyberdenBundle\Model\Setting Clone of current object.
     * @throws PropelException
     */
